...
alias gl='git log --oneline --graph --date-order --decorate --pretty=format:"%Creset %Cgreen%h %C(reset)%C(cyan)%x1b[s%an%x1b[u%x1b[8C%Creset%C(bold white) %s %C(bold black)(%ar)%C(yellow)%d %C(red)%ad" --branches'
